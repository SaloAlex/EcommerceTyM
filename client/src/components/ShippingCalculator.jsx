...
const ShippingCalculator = ({ onShippingAccepted }) => {
  ...
  return (
    <div className="bg-gray-200 p-6 rounded-lg shadow-lg text-gray-800 max-w-xl mx-auto">
      <h3 className="text-xl font-bold text-black mb-4 text-center">Calculadora de Envío</h3>

      <div className="mb-6">
        <label className="block text-gray-700 font-semibold mb-2" htmlFor="zone">
          Selecciona tu zona:
        </label>
        <select
          id="zone"
      ...
      <button
        ...
        className={`w-full px-6 py-2 bg-purple-600 text-white font-bold rounded-lg shadow-lg transition duration-300 ease-in-out transform hover:scale-105 ${
          loading || !selectedZone ? 'opacity-50 cursor-not-allowed' : 'hover:bg-purple-700'
        }`}
      ...
      {shippingCost !== null && !shippingAccepted && (
        <div className="mt-6">
          <p className="text-lg text-black font-semibold">El costo de envío es: ${shippingCost}</p>
      ...
      {shippingAccepted && (
        <p className="text-lg text-black font-semibold mt-4">El costo de envío ha sido añadido al total.</p>
      )}
      ...
      {showHelpModal && (
        <ShippingModal onClose={handleCloseHelpModal}>
          <div className="p-6 text-black">
            <h3 className="text-xl font-bold mb-4">¿Cómo saber de qué zona sos?</h3>
            <p className="mb-2">
              <strong>CABA:</strong> Si resides en la Ciudad Autónoma de Buenos Aires.
            </p>
            <p className="mb-2">
              <strong>GBA1:</strong> Localidades cercanas a la capital, como San Isidro, Vicente López, San Martín, 
              Morón, Lanús, Lomas de Zamora, entre otros.
            </p>
            <p>
              <strong>GBA2:</strong> Localidades más alejadas como Tigre, Escobar, Merlo, Moreno, Quilmes, Berazategui, 
              Florencio Varela, entre otros.
            </p>
          </div>
        </ShippingModal>
      )}
